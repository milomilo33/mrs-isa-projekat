<template>
  <Table  title="Dermatologists"  :addModal="addModal" :dermatologistss="dermatologistss" :fields2="fields2" :isHidden="isHidden" :items="items" :fields="fields" :infoModal="infoModal" :errorModal="errorModal"  @InfoP ="InfoP" @DeleteOne ="DeleteOne" @AddDermatologist ="AddDermatologist" @OnRowSelected ="OnRowSelected" @check="check"></Table>
</template>
<script>
  ...
  data() {
    return {
      ...
      fields: [{key:'name',sortable:true, label:'Name'}, {key:'lastName',sortable:true, label:'Last name'},{key: 'email',sortable:true, label:'Email'}, {key:'phoneNumber',sortable:true, label:'Phone number'},'info',{key:'update', label:'Edit'},'delete'],
        ...
        isHidden : false,
        dermatologistss:[],
        fields2 : [{key:'name', label:'Name'}, {key:'lastName', label:'Last name'},{key: 'email', label:'Email'}, {key:'phoneNumber', label:'Phone number'}],
       selected :{},
       hours : [],
   ...
   mounted(){
    var self = this;
    self.axios
      .get("http://localhost:8081/api/pharmacy/dermatologists/1")
      ...
      self.axios
      .get("http://localhost:8081/api/dermatologist/all")
      .then(function (response) {
        for(var i = 0;i<response.data.length;i++){
           if(self.items.filter((e) => e.email === response.data[i].email).length == 0
          ) {
          var item = {};
          item.name = response.data[i].name;
          item.lastName = response.data[i].lastName;
     ...
     check(item){
        ...
        if (idx != -1) {
           ...
           this.axios.put(
          "http://localhost:8081/api/dermatologist/updateDermatologist/"+this.selected.email,
          {
      ...
      DeleteOne(item,button){
        var self = this;
          self.axios.delete("http://localhost:8081/api/dermatologist/"+item.email+"/1")
      .then(function(response){
